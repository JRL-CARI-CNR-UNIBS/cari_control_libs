...
class RigidRoboticSystem(MechanicalSystem):
    def __init__(self, st, model_name):
        """
        Initialize the rigid robotic system with a given sampling period and model.
        """
    ...
    def state_function(self, x, u, t):
        ...
        """
        q = x[:self.njoints]  # Link position
        qd = x[self.njoints:2*self.njoints]
        ...
        q_numpy = np.array(q)
        qp_numpy = np.array(qd)
        qpp_numpy = np.zeros_like(q)
        ...
        qdd = np.linalg.solve(inertia, u - self.motor_viscous_term * qd - self.motor_coulomb_term * np.tanh(100.0 * qd))
